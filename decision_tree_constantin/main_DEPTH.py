...
#data_set_name = "mushroomsOneHot.csv"
#data_set_name = "mushrooms.csv"

# load data
if data_set_name == "mushrooms.csv":
    X, Y, feature_names, codeing = dataHandler.read_data(f_name = data_set_name,encode=True)
else:
    X, Y, feature_names, codeing = dataHandler.read_data(f_name = data_set_name,encode=False)

if data_set_name == 'wdbc.csv':
    feature_types = ['c']*X.shape[1]
elif data_set_name == 'mushrooms.csv' or data_set_name == 'mushroomsOneHot.csv':
...
for depth in depth_spacing:
    ...
    for iteration in range(10):
        ...
        learner = AdaBoost(X_train, Y_train,ensSize=depth+39)  
        #learner = BaggedLearner(X_train, Y_train,feature_types,feature_names,max_depth = 1,ensSize=depth,random_splits = True,isCat=False)
